...
class ConformalQuantileTimeSeriesRegressor(BaseConformalTimeSeriesRegressor):
    ...
    intervals : tuple, list, or mapping
        ...
        To associate an optional central forecast with each pair, pass a dictionary whose keys
    ...
    interval_medians_ : Dict[Tuple[str, str], Optional[str]]
        Mapping from each normalized interval pair to its optional central-forecast
        column. The re-scaled value has no separate conformal coverage guarantee.
    ...
    def _normalize_intervals(
        ...
        if isinstance(intervals, Mapping):
            if not all(
                isinstance(pair, tuple) and len(pair) == 2 for pair in intervals
            ):
                raise ValueError(
        ...
        configured_medians = [
            median for median in medians.values() if median is not None
        ]
    ...
    def predict_interval(
        ...
        """Apply conformal correction and optionally re-scale central forecasts.

        Generates calibrated interval columns named `<low_col>-cqr` and `<high_col>-cqr`.
        If a central forecast was associated with an interval pair during initialization,
        this also outputs a `<median_col>-cqr` column that preserves its original relative
        position inside the interval. This re-scaling is a convenience transformation;
        only the interval bounds receive a conformal coverage guarantee.
        ...
        for low_col, high_col in self.intervals_:
            ...
            if median_col is not None:
                if median_col not in pred_df.columns:
                    raise KeyError(
                        f"Median column '{median_col}' was not found in forecast output."
                    )
                ...
                skew_ratio = np.clip((q_median - q_low) / base_spread, 0.0, 1.0)
                pred_df[f"{median_col}-cqr"] = lower_bound + skew_ratio * cqr_spread
