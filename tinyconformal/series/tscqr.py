...
class ConformalQuantileTimeSeriesRegressor(BaseConformalTimeSeriesRegressor):
    ...
    ------------------------
    All quantile columns supplied in `interval_cols` MUST follow the strict pattern:
    ...
    horizon : int
        Forecast horizon H.
    interval_cols : Tuple[str, str] or List[Tuple[str, str]]
    ...
    def __init__(
        ...
        learner: BaseEstimator,
        horizon: int,
        interval_cols: Union[Tuple[str, str], List[Tuple[str, str]]],
        n_windows: int = 3,
        alpha: float = 0.05,
        ...
        self.interval_pairs_: List[Tuple[str, str]] = self._normalize_interval_cols(
            interval_cols
        )
    ...
    def _normalize_interval_cols(
        self, interval_cols: Union[Tuple[str, str], List[Tuple[str, str]]]
    ) -> List[Tuple[str, str]]:
        """Normalizes and validates interval_cols input using Regex pattern matching."""
        if isinstance(interval_cols, tuple) and len(interval_cols) == 2:
            pairs = [interval_cols]
        elif isinstance(interval_cols, list) and all(
            isinstance(p, (tuple, list)) and len(p) == 2 for p in interval_cols
        ):
            pairs = [tuple(p) for p in interval_cols]
        else:
            raise ValueError(
                "interval_cols must be a tuple of 2 column names (low, high) "
