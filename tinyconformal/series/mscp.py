...
class ConformalDistributionTimeSeriesRegressor(
    ...
    directly with empirical signed residuals defined as $R_{t,h} = \\hat{y}_{t,h} - y_{t,h}$.
    By computing low and high empirical quantiles ($q_{low}, q_{high}$) of these residuals across
    calibration windows, the prediction bounds are derived by inverting the nonconformity score:
    $$[\\hat{y}_{t,h} - q_{high}, \\hat{y}_{t,h} - q_{low}]$$
    ...
    def _compute_bounds(
        ...
        1. Quantile Adjustment: Given significance level $\\alpha$ and calibration count $n$,
           finite-sample adjusted quantiles $q_{low}$ and $q_{high}$ are computed using
           $\\frac{\\alpha}{2} - \\frac{1}{2n}$ and $1 - \\frac{\\alpha}{2} + \\frac{1}{2n}$.
        2. Empirical Residual Quantiles: Nonconformity scores $R = \\hat{y} - y$ sliced to horizon $h$
           yield per-step empirical residual thresholds ($q_{low, h}$ and $q_{high, h}$) along axis 0.
        3. Array Tiling: Since predictions $\\hat{y}$ are flattened across series, threshold vectors
           of size $h$ are repeated $n\\_series$ times using `np.tile`.
        4. Inversion Formula: The prediction bounds invert the nonconformity definition:
           - Lower Bound: $\\hat{y} - q_{high, h}$ (Subtracting the upper residual bound)
           - Upper Bound: $\\hat{y} - q_{low, h}$ (Subtracting the lower residual bound)
        """
