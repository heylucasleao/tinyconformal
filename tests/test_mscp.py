...
def test_coverage_rate():
    ...
    lower = np.array([8.0, 12.0, 18.0, 26.0])
...
def test_sample_correction_finite_bounds(mock_point_learner):
    ...
    assert pytest.approx(low_q, abs=0.0001) == 0.0
    assert pytest.approx(high_q, abs=0.0001) == 1.0
...
def test_fit_rejects_invalid_calibration_parameters(
    ...
    alpha = value if parameter == "alpha" else 0.05
    fit_kwargs = {"horizon": 2, "n_windows": 2}
    if parameter != "alpha":
        fit_kwargs[parameter] = value
    cdr = MultiStepConformalTimeSeriesRegressor(learner=mock_point_learner, alpha=alpha)
    with pytest.raises(ValueError, match=message):
        cdr.fit(sample_distribution_data, **fit_kwargs)
...
def test_mscp_custom_column_names(mock_point_learner):
    ...
    cdr = MultiStepConformalTimeSeriesRegressor(learner=learner)
    cdr.id_col = "series_id"
    cdr.time_col = "date_time"
    cdr.target_col = "target_val"
    cdr.nexcp = True
    cdr.decay = 0.99
    cdr.weighted_refit = True
    cdr.horizon = 2
...
def test_mscp_bounds_are_calibrated_by_unique_id(mock_point_learner):
    ...
    cdr = MultiStepConformalTimeSeriesRegressor(learner=mock_point_learner, alpha=0.5)
    cdr.id_col = "unique_id"
    cdr.time_col = "ds"
    cdr.target_col = "y"
    cdr.nexcp = True
...
def test_fit_separates_static_and_dynamic_features(
    ...
    cdr = MultiStepConformalTimeSeriesRegressor(learner=mock_point_learner).fit(
        df, static_features=["region"], n_jobs=1, horizon=2, n_windows=2
    )
...
def test_mscp_preserves_fractional_coverage_in_column_names(
    ...
    cdr = MultiStepConformalTimeSeriesRegressor(
        learner=mock_point_learner, alpha=0.055
    ).fit(sample_distribution_data, horizon=2, n_windows=2)
    pred_df = cdr.predict_interval(h=2)
...
def test_nexcp_weighted_refit_passes_internal_weight_column(
    ...
    MultiStepConformalTimeSeriesRegressor(learner=mock_point_learner).fit(
...
def test_nexcp_weighted_refit_requires_weight_col_support(sample_distribution_data):
    ...
    cdr = MultiStepConformalTimeSeriesRegressor(learner=LearnerWithoutWeights())
    cdr.id_col = "unique_id"
    cdr.time_col = "ds"
    cdr.target_col = "y"
    cdr.nexcp = True
